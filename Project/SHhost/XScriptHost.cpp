...
{
	/*
	if(NULL == pszFileName || 256 < wcslen(pszFileName))
		return S_FALSE;	
	HRESULT hr;
	if( !m_spDocument )
	{
		hr = m_spDocument.CoCreateInstance( CLSID_DOMDocument );		ATLASSERT( SUCCEEDED(hr) );
		if( FAILED(hr) )
			return hr;
	}
	CComBSTR bstrFileNmae=pszFileName;
	MakePath(bstrFileNmae);
	VARIANT_BOOL vb;
	hr = m_spDocument->load( CComVariant(bstrFileNmae), &vb );			ATLASSERT( SUCCEEDED(hr) );
	if( FAILED(hr) || !vb )
		return hr;
	//CComBSTR szFilePath ="ttc" ;
	//m_spDocument->get_url(&szFilePath);
	//if(!PathFileExistsW(L"\\\\192.168.13.159\\public\\xmp\\2.3.0.170\\XMPSetup_2.3.0.170Log.exe"))
		//return E_UNEXPECTED;
	*/
...
{
	CComBSTR bstrScript =  L"if(typeof onload == 'function') onload()";
...
HRESULT CXScriptHost::CreateScriptDebugger()
{
	...
	if(SUCCEEDED(hr))
	{
		hr = mpDebugApp->SetName(L"XScriptHost");
	}
	if(SUCCEEDED(hr))
	{
