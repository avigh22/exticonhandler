...
inline BOOL CCmdlineParser::_Parser()
{
	...
	if(lpwszItem)
	{
		...
		if(0 == _wcsicmp(szModPath, lpwszItem) || 0 == _wcsicmp(pszFileName, lpwszItem) || 
			(pszFileName2 && 0 == _wcsicmp(pszFileName, pszFileName2))) //xmp.exe
			i = 1;
	}
...
{
	/* 
			if(bstrKey==(CComBSTR)"play" || bstrKey==(CComBSTR)"dvd" || bstrKey==(CComBSTR)"addtolist" || bstrKey==(CComBSTR)"addtolistex"
				|| bstrKey==(CComBSTR)"cd" || bstrKey==(CComBSTR)"embedding" || bstrKey==(CComBSTR)"mini" 
				|| bstrKey==(CComBSTR)"s" || bstrKey==(CComBSTR)"silent" || bstrKey==(CComBSTR)"u" || bstrKey==(CComBSTR)"update"
				|| bstrKey==(CComBSTR)"a" || bstrKey==(CComBSTR)"associate" || bstrKey==(CComBSTR)"v" || bstrKey==(CComBSTR)"au"
				|| bstrKey==(CComBSTR)"smstartup" || bstrKey==(CComBSTR)"runxmp") //特殊命令(没有子参数)
 
		*/										
	//加如下这个是为防止 /play "d:\a.wmv" /dvd 这种方式的出现
	LPWSTR ppszKeys[] = {L"play", L"dvd", L"addtolist", L"addtolistex", L"cd", L"embedding", L"mini"
												L"s",L"silent",L"u",L"update",L"a",L"associate",L"v",L"au",L"smstartup",L"runxmp",};
